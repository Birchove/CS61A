...
from utils import split,lower,remove_punctuation
def about(keywords: list[str]):
    ...
    def check(sentence:str):
        check_list=[lower(remove_punctuation(i)) for i in split(sentence)]
        for i in keywords:
            if i in check_list:
                return True
        return False
...
def memo_diff(diff_function):
    ...
    def memoized(typed, source, limit):
        ...
        if key not in cache:
            ...
            cache[key]=result
            return result[0]
        elif limit>cache[key][1]:
            result=(diff_function(typed,source,limit),cache[key][1])
            cache[key]=result
            return result[0]
        else:
            return cache[key][0]
...
def autocorrect(typed_word: str, word_list: list[str], diff_function, limit: int) -> str:
    ...
    if typed_word in word_list:
        return typed_word
    diff=min([diff_function(typed_word,word,limit) for word in word_list ],key=abs)
    if diff>limit:
        return typed_word
    else:
        return [word for word in word_list if diff_function(typed_word,word,limit)==diff][0]
    # END PROBLEM 5
...
def furry_fixes(typed: str, source: str, limit: int) -> int:
    ...
    """
    # BEGIN PROBLEM 6
    def helper(typed_in: str, source_in: str, limit_in: int):
        if limit_in>limit:
             return limit_in+1
        if typed_in=="" or source_in =="":
            return abs(len(typed_in)-len(source_in))
        if typed_in[0]==source_in[0]:
            return helper(typed_in[1:],source_in[1:],limit_in)
        else:
            return helper(typed_in[1:],source_in[1:],limit_in+1)+1
...
def report_progress(typed: list[str], source: list[str], user_id: int, upload) -> float:
    ...
    """
    # BEGIN PROBLEM 8
    def helper(typed_in,source_in):
        if typed_in==[] or source_in==[]:
            return 0
        if typed_in[0] != source_in[0]:
            return 0
        else:
            return helper(typed_in[1:],source_in[1:])+1
...
def fastest_words(words_and_times: dict) -> list[list[str]]:
    ...
    for w in word_indices:
        # 初始化为玩家 0
        min_player = 0
        min_time = get_time(times, 0, w)
        for p in range(1, len(times)):
            t = get_time(times, p, w)
            if t < min_time:
                min_time = t
                min_player = p
        result[min_player].append(words[w])
    return result    
    # END PROBLEM 10

#fastest_words的一种实现:
#return [[words[w] for w in word_indices if get_time(times,p,w)==min(get_time(times,p,w) for p in player_indices) and all(get_time(times, q, w) != get_time(times, p, w) for q in range(p))]for p in player_indices]
