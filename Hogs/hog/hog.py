...
def boar_brawl(player_score, opponent_score):
    ...
    """
    # BEGIN PROBLEM 2
    digit_1=opponent_score//10%10
    digit_2=player_score%10
    res=abs(digit_1-digit_2)*3
    if res==0:
        return 1
    else:
        return res
...
def max_scoring_num_rolls(dice=six_sided, times_called=1000):
    ...
    temp=0
    for i in range(1,11):
        dice_now=make_averaged(roll_dice,times_called)(i,dice)
        temp=max(temp,dice_now)
        total_i[i]=dice_now
...
def final_strategy(score, opponent_score):
    ...
    1.因为BB规则可以预料结果，所以每次都优先判断BB规则是否成立，BB规则增值最大可达27(不考虑SF规则)
    2.而当BB规则只能得到1或3时（考虑到SF，我们将阈值设置为8），我们不如去掷骰子
    3.如果我们当前小幅落后于对方（<=10），那么倾向于稳妥，直接选择max_scoring_num_rolls
    4.如果我们现在大幅落后于对方，那么考虑冒险。
    以六面骰子为例，如果掷10次骰子，出现1的概率为0.838494，9对应0.806194，8次对应0.767432，7次对应0.720918
    那我们就掷8个骰子
    5.如果我们小幅领先或与对方得分相同，同样考虑冒险扩大优势
    6.如果我们大幅领先对方，那么稳妥
    7.以上规则均先考虑BB规则，因为BB规则可直接得知结果
    """
    # BEGIN PROBLEM 12
    if sus_strategy(score,opponent_score,threshold=8)==0:
        return 0
    if opponent_score-score<=10 and opponent_score-score>0:
        return 5
    if opponent_score-score>10:
        return 8
    if score-opponent_score<=10 and score-opponent_score>=0:
        return 8
